...
# PoolCapのObjectIDを抽出
pool_object_id=$(echo "$output1" | grep -A 30 "Created Objects:" | grep -B 3 "PoolCap" | grep "ObjectID:" | head -n 1 | awk '{print $4}')

echo "PackageID: $package_id"
echo "Pool ObjectID: $pool_object_id"

# initialize_poolを実行
echo "initialize_poolを実行します"
sui client ptb --move-call "${package_id}::pool::initialize_pool" @${pool_object_id} @${coin_object_id} 11 --gas-budget 10000000
